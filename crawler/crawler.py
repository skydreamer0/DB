...
# 資料庫和 JSON 檔案路徑
db_path = './shared_data/inventory.db'
json_path = './shared_data/drug.json'
...
def create_database():
    try:
        ...
        # 新增表格來存儲爬蟲結果，加入 barcode 和 location 欄位
        cursor.execute('''
...
def update_database_with_scraping_results(conn, cursor, barcode, location, results):
    try:
        cursor.execute('''
            INSERT OR REPLACE INTO drug_info
            (barcode, location, chinese_name, english_name, ingredients, license_number,
        ...
        conn.commit()
        logging.info(f"成功更新資料庫中 barcode {barcode} 和 location {location} 的資料")
    except Exception as e:
...
def process_all_barcodes():
    ...
    try:
        cursor.execute("SELECT barcode, location FROM inventory")
        records = cursor.fetchall()

        for record in records:
            barcode = record[0]
            location = record[1]
            logging.info(f"正在處理 barcode: {barcode}, location: {location}")
            results = perform_search(barcode)
            if results:
                update_database_with_scraping_results(conn, cursor, barcode, location, results)
            else:
